...
### 문제 정의
기존 AI 글쓰기 도구들은 세 가지 함정에 빠져 있다:

**함정 1: AI가 주도한다**
작가가 설정해도 AI가 자의적으로 캐릭터를 만들고, 감정선을 바꾸고, 새로운 설정을 덧붙인다.
InkOS에서 겪은 일: bible.md에 없는 "전남친", "부산 사투리", "비자 문제"를 AI가 스스로 만들어냄.

**함정 2: 도구가 작가를 제한한다**
특정 장르, 특정 언어, 특정 모델에 종속된다.
InkOS에서 겪은 일: zh/en만 지원, urban 장르에만 맞춰진 리뷰 기준, reasoning 모델 미대응.

**함정 3: 전체를 한 번에 만들려 한다**
한 번의 실행으로 전체 소설을 생성하려 해서, 중간 수정이 어렵다.
InkOS에서 겪은 일: short run은 한 방에 끝나서 챕터별 수정 불가, book create는 너무 무거움.
...
| 항목 | InkOS의 문제 | Essai의 해결 |
|------|-------------|-------------|
| 언어 | zh/en 하드코딩 | config.language → 프롬프트에 주입 |
| 장르 | 내장 프로파일 강제 | bible.md에서 작가가 정의 |
| 설정 | AI가 자의적 추가 | "bible에 없으면 쓰지 마" 강제 |
| 파이프라인 | 7단계 고정 | write 1화 = 1번 API 호출 |
| 리뷰 | 강제 (거절 시 재생성) | 선택적 (기본 OFF) |
| 수정 | 단편 불가 | 모든 챕터 개별 재생성 |
| 모델 | reasoning 미대응 | thinking_enabled 토글 |
| 복잡도 | 857 패키지 | 최소 의존성 |
| 투명성 | 불투명 | context 미리보기 명령 |
